...
export function StorageScreen() {
  ...
  useFocusEffect(useCallback(() => { const subscription = BackHandler.addEventListener('hardwareBackPress', back); return () => subscription.remove(); }, [back]));
  const more = async () => {
  ...
  return <Screen onRefresh={refresh} refreshing={loading}>
    <PageHeader eyebrow="SERVIDOR" title="Storage" right={local ? <StatusBadge status="warning" label="SIMULADO" /> : undefined} />
    ...
    {!location && !loading && !error && disks.map(disk => <Card key={disk.id} style={styles.disk}>
      ...
      {disk.volumes.map(volume => {
        ...
        return <View key={volume.id} style={styles.volume}>
          ...
          <View accessibilityRole="progressbar" accessibilityLabel={`Uso de ${volume.name}`} accessibilityValue={percent === null ? { text: 'Uso no disponible' } : { min: 0, max: 100, now: Math.round(percent) }} style={styles.track}><View style={{ height: 6, width: `${percent ?? 0}%`, borderRadius: 3, backgroundColor: percent !== null && percent > 90 ? colors.orange : colors.green }} /></View>
