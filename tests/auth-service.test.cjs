...
test('login and refresh match the mobile server JSON parser contract',async()=>{
  ...
  const requests=[];
  const scope={exports:{},AbortController,setTimeout,clearTimeout,require:id=>{
    if(id==='expo-secure-store')return {getItemAsync:async key=>values.get(key)||null,setItemAsync:async(key,value)=>values.set(key,value),deleteItemAsync:async key=>values.delete(key)};
    ...
    if(url.endsWith('/login')){
      ...
      assert.ok(request.body.data.deviceId.length>=8);assert.equal(request.body.data.platform,'android');
  ...
  assert.equal(await scope.exports.authService.refresh(),true);assert.equal(scope.exports.tokenStore.getAccessToken(),'new-access');assert.equal(requests.length,2);
});
